...
// This will generate a proper PDF file
...
function generatePDF($reportType, $reportId) {
    try {
        ...
        $html = generatePDFHTML($reportType, $reportData, $report);
        
        // Create a temporary HTML file
        $tempFile = tempnam(sys_get_temp_dir(), 'report_') . '.html';
        file_put_contents($tempFile, $html);
        
        // Use wkhtmltopdf if available, otherwise return HTML for browser PDF conversion
        if (function_exists('shell_exec') && shell_exec('which wkhtmltopdf')) {
            $pdfFile = str_replace('.html', '.pdf', $tempFile);
            $command = "wkhtmltopdf --page-size A4 --margin-top 20mm --margin-bottom 20mm --margin-left 20mm --margin-right 20mm '$tempFile' '$pdfFile'";
            shell_exec($command);
        ...
        // Fallback: Return HTML that can be converted to PDF by browser
...
function generatePDFHTML($reportType, $reportData, $report) {
    $html = '<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
